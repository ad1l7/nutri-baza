...
Olive напрямую из iiko, после чего промежуточный API Olive
(`/api/v1/labels/products/`) закрывается.
...
- количества перемножаются по дереву (`amountOut → amountMiddle → amountIn`),
  одинаковое сырьё суммируется, сортировка **по убыванию массы** — порядок
  ингредиентов на этикетке обязан идти от основного к второстепенному;
- защита от циклов в дереве — set `visited`;
...
1. **Лицензии iiko Server.** Токен `/resto/api/auth` занимает слот лицензии
...
2. **Одновременные синхронизации двух систем.** Olive продолжит синкать iiko
...
3. **Rate limit Cloud.** На 429 и 503 — пауза и повтор (в референсе
...
4. **Частота.** Раз в сутки ночью + кнопка «вручную» — за глаза: составы
   меняются редко. Чаще раза в час не ходить вообще.
5. **Смерть v1-авторизации ~29.08.2026.** Только `/api/v2/access_token`
   с `appId`+`clientSecret`. Токен протухает (~1 час) — по 401 получать новый
   и повторять запрос.
6. **Ничего не удалять автоматически.** Блюдо пропало из меню iiko — пометьте
...
7. **`null` ≠ 0.** Незаполненные КБЖУ/вес приходят как null — печатать «—»,
   не ноль и не падать.
8. **Пустой состав = блюдо не для печати.** Если после техкарт и description
   состав пуст — не показывать блюдо в списке печати (Olive фильтрует именно так).
9. **Один сбой ≠ падение синка.** Ошибка на одном блюде логируется и
...
Пока API Olive жив, используйте его как **эталон для сверки** — это главный
инструмент приёмки:

1. ERP реализует прямой пайплайн (разделы 4–5) в отдельную таблицу или
   отдельный source, не трогая работающую печать.
2. Прогон сверки: для каждого блюда сравнить свой результат с
   `GET https://olive-nutri-baza.kz/api/v1/labels/products/` (токен тот же,
   что при первой интеграции). Поля: `name_print`, `composition`,
   `net_weight_g`, все КБЖУ. Расхождений быть не должно — словарь и логика
   идентичны; каждое расхождение = ошибка переноса, разбирать до нуля.
3. Переключить печать на прямой источник. День-два понаблюдать.
4. Сообщить владельцу Olive — и API `/api/v1/labels/` будет закрыт
   (выключается одной строкой на сервере, код можно удалить позже).

До шага 4 старый канал работает — откат в любой момент это просто
переключение источника обратно.
