...
_SYSTEM = (
    ...
    "4. Учитывай пожелания пользователя (предпочтения, ограничения, бюджет и т.п.).\n"
    "4.1. Если пожелание запрещает ингредиент («без сахара», «без свинины», "
    "«без молочного»), проверяй поле composition КАЖДОГО блюда и исключай все, "
    "где такой ингредиент встречается — даже если по названию блюдо кажется "
    "подходящим. Названия ненадёжны: сахар часто стоит в составе салатов и "
    "горячих блюд. Учитывай очевидные синонимы и формы (сахар — сироп, "
    "сахарная пудра, мёд; молочное — молоко, сливки, сыр, творог, йогурт).\n"
    "4.2. В reasoning отдельной строкой напиши, по какому ограничению фильтровал "
    "состав и сколько блюд из-за этого отсеял.\n"
...
def _build_catalog(products) -> list:
    ...
    Состав обязателен: без него нельзя выполнить пожелания вида «без сахара» —
    по названию ингредиент не виден. Каталог из-за состава весит примерно вдвое
    больше (около 38 тысяч токенов на 289 блюд), но на скорость сборки это
    почти не влияет: её определяет длина ответа, а не запроса."""
    catalog = []
    for p in products:
...
def generate_ration(wishes: str, products, norms, meal_times=None) -> dict:
    ...
    user_prompt = (
        ...
        f"Каталог блюд (КБЖУ и себестоимость — на порцию, composition — состав):\n"
